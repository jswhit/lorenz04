...
import sys, time, os
from lorenz04 import Lorenz04, cartdist, lgetkfms_bloc, gaspcohn
...
covlocal_model = np.empty((nlscales,nx,nx),np.float64)
for nlscale in range(nlscales):
    for n in range(nx):
        dist = cartdist(x[n],x,models[0].model_size)
        covlocal_model[nlscale,n,:] = gaspcohn(dist/hcovlocal_scales[nlscale])

#evals, evecs = eigh(covlocal_model[0],driver='evd')
#evals = evals.clip(min=np.finfo(evals.dtype).eps)
#sqrtcovlocal11 = np.dot(evecs, np.diag(np.sqrt(evals)))
#if nlscales == 2:
#    evals, evecs = eigh(covlocal_model[1],driver='evd')
#    evals = evals.clip(min=np.finfo(evals.dtype).eps)
#    sqrtcovlocal22 = np.dot(evecs, np.diag(np.sqrt(evals)))
#    covlocal12 = crossbandcov_facts[0]*np.dot(sqrtcovlocal11,sqrtcovlocal22.T)
#    covlocal = np.block([[covlocal_model[0], covlocal12],[covlocal12.T, covlocal_model[1]]])
...
# compute sqrt of localization matrix for each local volume.
sqrtcovlocal_local = []
for n in range(nx):
    ...
    nrow = 0
    for nn in indx:
        dist = cartdist(x[nn],x[indx],models[0].model_size)
        covlocal11[nrow,:] = gaspcohn(dist/hcovlocal_scales[0])
        nrow += 1
    evals, evecs = eigh(covlocal11,driver='evd')
    evals = evals.clip(min=np.finfo(evals.dtype).eps)
    sqrtcovlocal11 = np.dot(evecs, np.diag(np.sqrt(evals)))
    if nlscales == 2:
        ...
        for nn in indx:
            ...
            nrow += 1
        evals, evecs = eigh(covlocal22,driver='evd')
        evals = evals.clip(min=np.finfo(evals.dtype).eps)
        sqrtcovlocal22 = np.dot(evecs, np.diag(np.sqrt(evals)))
        covlocal12 = crossbandcov_facts[0]*np.dot(sqrtcovlocal11,sqrtcovlocal22.T)
        ...
        evals = evals.clip(min=np.finfo(evals.dtype).eps)
        neig = 1
        for i in range(1,npts*nlscales):
            percentvar = evals[-i:].sum()/evals.sum()
            if percentvar > percentvar_cutoff: # perc variance cutoff truncation
                neig = i
                break
...
kmax = (nx//2)+1
covlocal_ob = np.empty((nobs,nx),np.float32)
...
for ntime in range(nassim):
    ...
    xob = x[indxob]
    # compute covariance localization function for each ob (using largest length scale to define local region)
    for nob in range(nobs):
        distob = cartdist(xob[nob],x,nc_climo.model_size)
        covlocal_ob[nob,:] = gaspcohn(distob/hcovlocal_scales[0])
